...
----------------------------------------------------------------------------------------------------
['johannes ackermann michael noukhovitch takashi ishida masashi sugiyama', 'introduction', 'preprint february', 'gradient regularization prevents reward hacking', 'background', 'reinforcement learning', 'proxy rewards', 'gradient regularization prevents reward hacking', 'gradient regularization gr', 'the model parameters are then updated as', 'accurate proxy rewards via gradient regularization', 'problem formulation and overview', 'gradient regularization improves proxy reward accuracy', 'gradient regularization prevents reward hacking', 'gradient regularization prevents reward hacking', 'reference reset as gradient regularization', 'gradient regularization prevents reward hacking', 'explicit gradient regularization', 'gr mitigates hacking reward models in rlhf', 'gradient regularization prevents reward hacking', 'gr prevents focus on easy rule based rewards', 'gradient regularization prevents reward hacking', 'gr mitigates hacking llm as a judge in rlvr', 'conclusion', 'gradient regularization prevents reward hacking', 'impact statement', 'gradient regularization prevents reward hacking', 'gradient regularization prevents reward hacking', 'gradient regularization prevents reward hacking', 'gradient regularization prevents reward hacking', 'a extended related work', 'b proof s', 'gradient regularization prevents reward hacking', 'the sub optimality of this action perturbation is', 'gradient regularization prevents reward hacking', 'we can rearrange this to', 'gradient regularization prevents reward hacking', 'for any two points x y b c r', 'gradient regularization prevents reward hacking', 'b bt loss lower bound based on sharpness', 'gradient regularization prevents reward hacking', 'figure implementation of finite difference gradient regularization with grpo in pytorch', 'b connection of gradient regularization and lipschitz continuity', 'c experiment details', 'c gradient regularization implementation', 'gradient regularization prevents reward hacking', 'c rlhf details', 'gradient regularization prevents reward hacking', 'c gradient regularization on alpaca farm', 'c reasoning experiments', 'table hyperparameters in reasoning experiments with gr', 'gradient regularization prevents reward hacking', 'table hyperparameters in reasoning experiments with kl penalty or no penalty', 'c llm judge setup', 'gradient regularization prevents reward hacking', 'd additional experiments', 'd gradient regularization and learning rate sweeps', 'd gradient regularization decreases bt loss', 'gradient regularization prevents reward hacking', 'd llm judge ablation', 'd kl penalty schedule', 'd steps per reset r', 'd reward hacking example', 'given values', 'gradient regularization prevents reward hacking', 'e reference resets in rlhf', 'e asymptotic analysis', 'if we insert k into this we obtain', 'gradient regularization prevents reward hacking']
